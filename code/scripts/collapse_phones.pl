...
my $collapse_repeats = 0;
if($ARGV[0] eq "--ignore-first-field") { $ignore_first_field = 1; shift @ARGV; }
if($ARGV[0] eq "--collapse-repeats") { $collapse_repeats = 1; shift @ARGV; }

my $symtab = shift @ARGV;

if(!defined $symtab) {
    die "Usage: collapse_phones.pl --ignore-first-field --collapse-repeats symtab [phoneme mapping] > output transcriptions\n";
}
