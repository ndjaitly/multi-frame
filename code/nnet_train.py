...
class nn(object):
    ...
    def create_nnet_from_def(self, def_file, data_dim=-1, target_dim=-1, 
        ...
        if input_mask is not None:
            assert(input_mask.shape[0] == data_dim)
            nn_def.layers[0].num_units = input_mask.shape[1] 
        ...
        for layer_num, layer_def in enumerate(nn_def.layers):
            ...
            if input_mask is not None and layer_num == 0:  layer.set_input_mask(input_mask)
            self._lst_layers.append(layer)
    ...
    def predict(self, data, unnormalized=False):
        num_pts = 1000
        predictions = zeros((self._lst_num_hid[-1], data.shape[1]))
        index = 0
        for layer_num, layer in enumerate(self._lst_layers): 
            layer.dropout = 0
    ...
    def train_one_epoch(self, data_src, eps_ratio=1.0, class_multipliers=None):
        cm_probs = cm.empty((1, self._batch_size))
        cm_correct = cm.empty((1, self._batch_size))
        ...
        cm_ones = cm.empty((1, self._batch_size)).assign(1)
        try:
            self.__cur_epoch += 1
        ...
        for  (cm_data, cm_labels) in \
            ...
            batch += 1
            num_pts += self._batch_size
            self.fwd_prop(cm_data)
            cm.compute_softmax_accuraccy(cm_predictions, cm_labels, 
                                         cm_probs, cm_correct)
            classif_err_sum += (self._batch_size -cm.vdot(cm_correct, cm_ones))
