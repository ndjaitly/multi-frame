...
class rnd_struct(ct.Structure):
    _fields_ = [('dev_rnd_mults', ct.POINTER(ct.c_uint)), 
                ('dev_rnd_words', ct.POINTER(ct.c_ulonglong))]
...
class CUDAMatrix(object):
    ...
    def reshape(self, shape):
        ...
        if err_code:
            raise generate_exception(err_code)
        # size of transposed should also change. So just update it.
        self.T = TransposedCUDAMatrix(self.mat)
    ...
    def fill_with_rand(self):
        ...
        """
        raise Exception, "You should be using cudamat_ext.fill_with_rand"
    ...
    def fill_with_randn(self):
        ...
        distribution.
        """
        raise Exception, "You should be using cudamat_ext.fill_with_randn"
